...
files = glob(ANA_DIR + "/allsky_scan/no_prior_versatile/high/tw{}/GRB180423A_batchSize*_batchIndex*_tw{}.npz".format(args.tw_in_second, args.tw_in_second))
...
for grb_idx in range(args.grb_name_idx_start, args.grb_name_idx_end):
    ...
    outfilename = "{}_tw{}_NTrial{}.npz".format(grb_name, 
                                                 args.tw_in_second,
                                                sTSs.shape[1])
    output_folder = cy.utils.ensure_dir(ANA_DIR+"/allsky_scan/with_prior_background/high/tw{}".format(args.tw_in_second))
    sparse.save_npz(output_folder+"/{}".format(outfilename) ,sTSs)
    print("\n===== Saved =====\n")
    
########################################## Low ##################################################################
print("\n===== Loading no-healpix background all-sky scan =====\n")
files = glob(ANA_DIR + "/allsky_scan/no_prior_versatile/low/tw{}/GRB180423A_batchSize*_batchIndex*_tw{}.npz".format(args.tw_in_second, args.tw_in_second))
files.sort(key=lambda x: int(x[x.find("batchIndex") + len("batchIndex"): x.find("_tw")]))
print("\n===== Done =====\n")
...
for grb_idx in range(args.grb_name_idx_start, args.grb_name_idx_end):
    ...
    outfilename = "{}_tw{}_NTrial{}.npz".format(grb_name, 
                                                 args.tw_in_second,
                                                sTSs.shape[1])
    output_folder = cy.utils.ensure_dir(ANA_DIR+"/allsky_scan/with_prior_background/low/tw{}".format(args.tw_in_second))
    sparse.save_npz(output_folder+"/{}".format(outfilename) ,sTSs)
    print("\n===== Saved =====\n")
#######################################################################################################################    
print("All done")